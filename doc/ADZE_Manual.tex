...
This file is space-separated in both layouts --- version 1.0 wrote it that way,
and one column per locus does not become easier to read under tabs --- but
\flag{--legacy} does change what happens to an undefined value. By default a
locus at which the statistic is undefined at this $g$ is written as
\texttt{NA}, which names the locus responsible: the reader can see which column
went missing and at which $g$ it did so.

\begin{Verbatim}[xleftmargin=1em,fontsize=\small]
POP_GROUPING G NUM_LOCI L0 L1 L2 L3 MEAN VAR STD_ERR
POP1 2 4 1.53571 1.60714 1 1.67857 1.45536 0.095557 0.154561
POP1 3 4 1.80357 2 NA 2.125 NA NA NA
\end{Verbatim}

\noindent Here \texttt{POP1} scored only two gene copies at \texttt{L2}, so
...
undefined at a given $g$ is written as \texttt{NA}. \flag{--tsv} names that
layout explicitly and is accepted for compatibility with scripts written
against earlier 2.0 builds, where it was not the default.
...
with no visible explanation. Asking for both layouts at once is refused rather
