...
Source is at \url{https://github.com/szpiech/ADZE}. Version 2.0 has no external
dependencies: a C++17 compiler is sufficient. OpenMP is used for
multi-threading when the toolchain provides it, and the program builds and runs
without it.
...
\begin{Verbatim}[xleftmargin=1em]
make -C src                  # serial
make -C src OPENMP=1         # GCC, or clang on Linux
make -C src OPENMP=1 OMPFLAGS="-Xpreprocessor -fopenmp" OMPLIBS=-lomp
                             # Apple clang, with libomp installed
\end{Verbatim}

\noindent CMake reports whether OpenMP was found at configure time. Without it,
\flag{--threads} is accepted but warns that it is being ignored.
...
The data file is expected in the same format used by \structureprog: one row
...
Rows are grouped by the value in the \flag{--group-col} label column;
groupings appear in output in the order they are first seen in the file. Any
label column can be used, so a file annotated with population, region and
continent can be analysed at any of those levels without being rewritten.
...
Running the same analysis from the distributed parameter file, which declares
\texttt{MAX\_G 8} and all four dimensions explicitly, gives identical results
for the loci and $g$ values that are defined:
\begin{Verbatim}[xleftmargin=1em]
adze example/small_paramfile.txt --tolerance 0.2
\end{Verbatim}
